...
-- 
-- 문제: creators 테이블에서 채널 설명(channel_description)이 등록되지 않은 (값이 NULL인) 
-- 크리에이터가 있는지 찾아보세요.
SELECT 
       creator_id
  FROM creators 
...
-- 주제: WHERE (종합 논리 조건)
-- 
-- 문제: users 테이블에서, 국적이 '영국'이거나 '프랑스'이면서, 동시에 사용자 이름에 
-- '사용자'라는 단어가 포함되지 않는 사용자를 조회하세요.
SELECT 
       username
  FROM users 
 WHERE (country = '영국'
