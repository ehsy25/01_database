...
-- 문제: creators 테이블에서 creator_id가 1, 3, 5, 7인 
-- 크리에이터의 채널명과 creator_id를 조회하세요.
SELECT 
 	   channel_name '채널명'
 	 , creator_id
  FROM creators 
...
-- 문제: 'IT/테크' 카테고리이거나 '교육' 카테고리이면서, 동시에 재생 시간이 1시간(3600초) 이상인 
-- 비디오의 **제목, 카테고리, 재생 시간(초)**을 조회하세요.
SELECT
 title '제목'
	 , category 'category'
...
-- 문제: 모든 비디오를 카테고리(오름차순) 순으로 먼저 정렬하고, 같은 카테고리 내에서는 
-- 조회수(내림차순)가 높은 순으로 정렬하여 비디오의 카테고리, 제목, 조회수를 조회하세요.
SELECT 
	   category '카테고리'
	 , title '제목'
	 , view_count
...
-- 문제: 크리에이터의 채널 설명(channel_description)에 '튜토리얼'이라는 단어가 
-- 포함되지 않은 채널의 이름을 조회하세요.
SELECT 
	   channel_name '채널 이름'
  FROM creators
...
-- 문제: 재생 시간(duration_seconds)이 10분(600초)에서 30분(1800초) 
-- 사이가 아닌 비디오의 제목과 재생 시간을 조회하세요.
SELECT 
 title '비디오 제목'
...
-- 재생 시간이 5분(300초) 이하인 경우를 모두 조회하세요. 
...
-- 문제: 모든 사용자를 user_id 순으로 정렬했을 때, 11번째부터 20번째까지(10건)의 
-- 사용자 이름과 이메일을 조회하세요.
SELECT 
	   username '사용자 이름'
	 , email '이메일'
...
SELECT 
	    username
  FROM  users
 WHERE (country = '영국' OR country = '프랑스') 
