...
SELECT title
	...
	, duration_seconds
FROM videos 
WHERE (category LIKE 'IT/테크' OR category LIKE '교육') AND duration_seconds >=3600; 
...
SELECT title
	...
	, duration_seconds
FROM videos
WHERE category LIKE '여행' AND view_count>=500000 OR category LIKE '음악' AND duration_seconds <=300;
...
SELECT username
FROM users
WHERE country  IN('영국','프랑스') AND username NOT LIKE "%사용자%";
